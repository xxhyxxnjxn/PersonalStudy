...
@Service
public class BithumbRevenueServiceImpl implements BithumbRevenueService{
	// 빗썸
	@Autowired
	...
	@Override
	@Transactional
	public void setAvgPrice_bithumb(String sitename, List<Report> bithumbReport, User user) throws ParseException { 
		...
		for (int i = report.size() - 1; i >= 0; i--) {
			...
			int x = reportRepository.updateReport(report.get(i).getOrderId(), report.get(i).getSite(), report.get(i).getTransactionDate(), report.get(i).getCurrency(), report.get(i).getType(), report.get(i).getPrice(), report.get(i).getUnits(), report.get(i).getBidUnitsCal(), 
...
//				}
	...
	@Override
	@Transactional
	public void setAvgPrice_bithumb_latest(String sitename, List<Report> bithumbReport_isNullEntity, List<Report> bithumbReport_latestRowEntity, User user) throws ParseException { 

		List<ReportDto> bithumbReport_latestRow = ReportDtoMapper.INSTANCE.toDto(bithumbReport_latestRowEntity);
		List<ReportDto> bithumbReport_isNull = ReportDtoMapper.INSTANCE.toDto(bithumbReport_isNullEntity);
		
		bithumbReport_latestRow = replaceNull(bithumbReport_latestRow);
		
		BigDecimal bid_totalPrice_cal= new BigDecimal(bithumbReport_latestRow.get(0).getBidTotalPriceCal()); 
		BigDecimal ask_totalPrice_cal = new BigDecimal(bithumbReport_latestRow.get(0).getAskTotalPriceCal());
		
		BigDecimal bid_units_cal= new BigDecimal(bithumbReport_latestRow.get(0).getBidUnitsCal()); 
		BigDecimal ask_units_cal = new BigDecimal(bithumbReport_latestRow.get(0).getAskUnitsCal()); 

		BigDecimal bid_totalPrice = new BigDecimal("0.0"); // price * units 구하는 식
		BigDecimal ask_totalPrice = new BigDecimal("0.0");

		BigDecimal bid_avgPrice = new BigDecimal(bithumbReport_latestRow.get(0).getBidAvgPrice()); 
		BigDecimal ask_avgPrice = new BigDecimal(bithumbReport_latestRow.get(0).getAskAvgPrice());

		BigDecimal bid_fee = new BigDecimal("0.0");
		BigDecimal ask_fee = new BigDecimal("0.0");
		
		BigDecimal bid_accUnits = new BigDecimal(bithumbReport_latestRow.get(0).getBidAccUnits());
		
		BigDecimal total_rate_cal= new BigDecimal(bithumbReport_latestRow.get(0).getTotalRateCal());
		BigDecimal cal = new BigDecimal(bithumbReport_latestRow.get(0).getCal()); 
		
		BigDecimal ask_accUnits = new BigDecimal(bithumbReport_latestRow.get(0).getAskAccUnits());
		BigDecimal askAccUnits= new BigDecimal("0.0");
		
		BigDecimal totalPriceCal= new BigDecimal("0.0");
		
		BigDecimal bidTotalPriceCal= new BigDecimal(bithumbReport_latestRow.get(0).getBidTotalPriceCal()); 
		BigDecimal bidUnitsCal= new BigDecimal("0.0");
		BigDecimal askTotalPriceCal= new BigDecimal("0.0");
		BigDecimal askUnitsCal= new BigDecimal("0.0");
		
		Report report_entity = null;
		List<ReportDto> report = setBithumbData_latest(bithumbReport_isNull, bithumbReport_latestRow);
		...
		for (int i = 0; i <report.size(); i++) {
			...
			if (report.get(i).getType().equals("매수")||report.get(i).getType().equals("입금")) {
				...
				if(i==0 && bithumbReport_latestRow.get(0).getType().equals("매도")) {
					bid_totalPrice_cal= new BigDecimal("0.0");
					bid_units_cal= new BigDecimal("0.0"); 
				}else if(i==0 && bithumbReport_latestRow.get(0).getType().equals("매수")) {
					ask_accUnits = new BigDecimal("0.0"); // 더해주자마자 바로 초기화
				}
					...
					if(i==0 && bithumbReport_latestRow.get(0).getType().equals("매수")) {
						ask_totalPrice_cal= new BigDecimal("0.0");
						ask_units_cal= new BigDecimal("0.0"); 
					}
...
//			}
	...
	public List<ReportDto> setBithumbData_latest(List<ReportDto> report, List<ReportDto> bithumbReport_latestRow) {
		...
		for (int i = 0; i <report.size(); i++) {
			...
			if(i == 0 && bithumbReport_latestRow.get(0).getAccUnits()!="0.0000") {
				cal_accUnits = new BigDecimal(bithumbReport_latestRow.get(0).getAccUnits());
				System.out.println("DD"+cal_accUnits);
			}
...
//		}
	...
	public List<ReportDto> replaceNull( List<ReportDto> bithumbReport_latestRow){
		if(bithumbReport_latestRow.get(0).getBidTotalPriceCal()==null) {
			bithumbReport_latestRow.get(0).setBidTotalPriceCal("0.0");
		}
		if(bithumbReport_latestRow.get(0).getAskTotalPriceCal()==null) {
			bithumbReport_latestRow.get(0).setAskTotalPriceCal("0.0");
		}
		
		if(bithumbReport_latestRow.get(0).getBidUnitsCal()==null) {
			bithumbReport_latestRow.get(0).setBidUnitsCal("0.0"); 
		}
		if(bithumbReport_latestRow.get(0).getAskUnitsCal()==null) {
			bithumbReport_latestRow.get(0).setAskUnitsCal("0.0"); 
		}
		
		if(bithumbReport_latestRow.get(0).getBidAvgPrice()==null) {
			bithumbReport_latestRow.get(0).setBidAvgPrice("0.0"); 
		}
		if(bithumbReport_latestRow.get(0).getAskAvgPrice()==null) {
			bithumbReport_latestRow.get(0).setAskAvgPrice("0.0");
		}
		
		if(bithumbReport_latestRow.get(0).getBidAccUnits()==null) {
			bithumbReport_latestRow.get(0).setBidAccUnits("0.0");
		}

		if(bithumbReport_latestRow.get(0).getTotalRateCal()==null) {
			bithumbReport_latestRow.get(0).setTotalRateCal("0.0");
		}
		if(bithumbReport_latestRow.get(0).getTotalRateCal()==null) {
			bithumbReport_latestRow.get(0).setTotalRateCal("0.0");
		}
		if(bithumbReport_latestRow.get(0).getCal()==null) {
			bithumbReport_latestRow.get(0).setCal("0.0"); 
		}
		if(bithumbReport_latestRow.get(0).getAskAccUnits()==null) {
			bithumbReport_latestRow.get(0).setAskAccUnits("0.0");
		}

		return bithumbReport_latestRow;
	}
}
