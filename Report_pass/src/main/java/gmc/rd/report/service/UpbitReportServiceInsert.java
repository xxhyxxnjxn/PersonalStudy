...
@Service
public class UpbitReportServiceInsert implements UpbitReportService{
		...
		if (apiDto == null) {
			...
			int page_cnt = 1;
				...
				if (deposit1.equals("[]")) {
					...
					System.out.println("deposits 값" + deposit1);
		...
		if (apiDto == null) {
			...
			int page_cnt = 1;
				...
				if (withdraw1.equals("[]")) {
					...
					System.out.println("withdraw1 값" + withdraw1);
					Type listType = new TypeToken<List<Deposit>>() {
					}.getType();
...
//				System.out.println(page_cnt + "페이지");
		...
		if (apiDto == null) {
			...
			if (upbitTransaction2.isEmpty()) {
				...
				for (int i = 0; i < upbitTransaction2.size(); i++) {
					...
					SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssX");
					Date date = format.parse(upbitTransaction2.get(i).getCreated_at());
					Calendar cal2 = Calendar.getInstance();
					cal2.setTime(date);
...
//						
//					}else if(compare < 0 ) {
						if (upbitTransaction2.get(i).getMarket().contains("BTC-")) {
							// System.out.println("삭제할 UUID : " + upbitTransaction2.get(i).getUuid());
				...
				for (int i = 0; i < upbitTransaction2.size(); i++) {
					try {
						...
						if(trades.size() == 0) {
							...
							for (int j = 0; j < trades.size(); j++) {
								trades.get(j).setCurrency(upbitTransaction2.get(i).getMarket());
								trades.get(j).setFee(upbitTransaction2.get(i).getPaid_fee());
								tradeslist.add(trades.get(j));
...
//	                     setAvgPrice_upbit(sitename, report2, user);
	...
	public void selectCandleStick(User user) throws JsonMappingException, JsonProcessingException {
		List<Report2> groupByCurrencyBankStateMent = reportRepository2.groupByCurrencyBankStateMent(user.getMemId());
		...
		for(int i=0;i<groupByCurrencyBankStateMent.size();i++) {
			...
			List<Report2> findByBankStateMent = reportRepository2.findByBankStateMent(user.getMemId(), groupByCurrencyBankStateMent.get(i).getCurrency());
