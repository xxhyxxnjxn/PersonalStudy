					...
					$.each(json.content, function(index, trade) {
						...
						$.ajax({
							...
							success: function(json) {
								...
								hiddenType = trade.type;

								if (hiddenType != before_hiddenType && hiddenType.toString() == "매수") {
									var html = "<tr class='tr_visible'><td style='text-align: right;'><a class='plus'>+</a><a class='minus' style='display:none'>-</a> " + num + "</td>" +
										//var html = "<tr><td style='text-align: right;'>" + num + "</td>" +
										"<td>" + trade.site + "</td>" +
										...
										"</td><td>" + report + "</td></tr>";
								} else if (hiddenType == before_hiddenType && hiddenType.toString() == "매수" && before_hiddenType.toString() == "매수") {
									var html = "<tr class = 'tr_hidden' style='display:none'><td style='text-align: right;'>" + num + "</td>" +
										//var html = "<tr><td style='text-align: right;'>" + num + "</td>" +
										"<td>" + trade.site + "</td>" +
										"<td>" + trade.currency + "</td>" +
										"<td id='dateId'>" + full + "</td>" +
									...
									var html = "<tr class='tr_visible'><td style='text-align: right;'>" + num + "</td>" +
										"<td>" + trade.site + "</td>" +
										"<td>" + trade.currency + "</td>" +
										"<td id='dateId'>" + full + "</td>" +
										"<td class=" + typeColor + ">" + trade.type + "</td>" +
...
function rerevenue(site) {
	...
	$.ajax({
		...
		success: function(j) {
			...
			console.log(j.state);
			if (j.state == false) {
				revenueTimer = setInterval(function() {
					revenueTime();
				}, 1000);
				$.ajax({
	...
	$.ajax({
		...
		success: function(json) {
			if (json.state == '0') {
				...
				document.getElementById('roadingStr').innerHTML = "수익률 계산 중입니다.";
			}
		},
		error: function(json) {
			alert('에러' + json.status + ',' + json.textStatus);
